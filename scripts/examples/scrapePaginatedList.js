...
import { getRandomMilliseconds, sleep } from '../../src/utils.js'
import { flagForbiddenWords } from '../../src/cleanupData.js'
...
const getDataFromDirectory = async (currentListings = [], targetURL = '') => {
  ...
  const { nextListings, nextPageURL } = await page.evaluate(
    async (targetPage) => {
      ...
      const currentPage =
        doc.querySelector('') // YOUR QUERY HERE
          ?.innerText ||
  ...
  for (const result of data) {
    ...
    try {
      ...
      const detailedResults = await page.evaluate(async (retrievalLink) => {
        ...
        const fullDescription = doc.querySelector(descriptionSelector)
          ?.innerHTML
...
for (let item of data) {
  const passesFilters = [
    item.fullDescription, // this includes the filters from the first pass
